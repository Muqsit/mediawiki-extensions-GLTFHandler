...
final class JSONSchema {
	...
	public static function validate( mixed $json, array $schema, array $base = [], int $flags = 0, int $code = GLTFParser::ERR_INVALID_SCHEMA ): void {
		...
		while ( isset( $buffer[$index] ) ) {
			...
			$directory_path = implode( "/", array_map( 'strval', $directory_cur ?? [] ) );
			gettype( $value ) === gettype( $expect ) || ( is_float( $expect ) && is_int( $value ) ) || throw new InvalidArgumentException( "Expected type of value at /{$directory_path} to be " . gettype( $expect ) . ", got " . gettype( $value ), $code );
